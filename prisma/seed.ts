...
async function main() {
  ...
  const senhaCliente = await bcrypt.hash(
    "Cliente@123",
    10
  );
  ...
  const xBurger = await prisma.produto.upsert({
    ...
    update: {
      nome: "X-Burger",
      descricao:
        "Hambúrguer, queijo, molho da casa e pão especial",
      categoria: "Lanches",
      preco: 32.9,
      custo: 12.5,
      ativo: true,
  ...
  console.log(
    "ADMIN   -> admin@restaurante.com / Admin@123"
  );
  console.log(
    "CLIENTE -> cliente@restaurante.com / Cliente@123"
  );
  console.log("");
  console.log(
  ...
  console.log(
    `Produto: ${xBurger.nome} (ID ${xBurger.id})`
  );
  console.log(
    `Produto: ${refrigerante.nome} (ID ${refrigerante.id})`
  );
}
