...
export async function consultarPontos(usuarioId: number) {
...
export async function registrarConsentimentoFidelidade(
  usuarioId: number
) {
  const usuario = await prisma.usuario.findUnique({
    where: {
  ...
  await registrarAuditoria({
    ...
    detalhes: {
      fidelidadeAtiva: true,
      consentimentoFidelidadeEm:
        usuarioAtualizado.consentimentoFidelidadeEm,
    },
  });
...
export async function resgatarPontos(
  ...
  const valorBeneficio =
    Math.floor(pontos / 100) * 10;

  if (valorBeneficio <= 0) {
    throw new Error("MINIMO_RESGATE_NAO_ATINGIDO");
  }

  const pontosConsumidos =
    Math.floor(pontos / 100) * 100;
  ...
  await registrarAuditoria({
    ...
    detalhes: {
      ...
      valorBeneficio,
      saldoRestante:
        usuarioAtualizado.pontosFidelidade,
    },
  });
...
export async function creditarPontosPedido(
  ...
  if (!pedido.usuario.fidelidadeAtiva) {
    return {
      pontosCreditados: 0,
      mensagem:
        "Usuário não aderiu ao programa de fidelidade.",
    };
  }
  ...
  if (pontos <= 0) {
    return {
      pontosCreditados: 0,
      mensagem:
        "O pedido não gerou pontos de fidelidade.",
    };
  }
