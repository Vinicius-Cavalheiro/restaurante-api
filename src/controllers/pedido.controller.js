import { criarPedido, atualizarStatusPedido, listarPedidos, } from "../services/pedido.service.js";
export async function list(req, res) {
    try {
        const canalPedidoParam = req.query.canalPedido;
        const canaisPermitidos = [
            "BALCAO",
            "APP",
            "DELIVERY",
        ];
        if (canalPedidoParam !== undefined &&
            (typeof canalPedidoParam !== "string" ||
                !canaisPermitidos.includes(canalPedidoParam))) {
            return res.status(400).json({
                error: "CANAL_PEDIDO_INVALIDO",
                message: "O canalPedido informado é inválido.",
            });
        }
        const pedidos = canalPedidoParam !== undefined
            ? await listarPedidos(canalPedidoParam)
            : await listarPedidos();
...
export async function updateStatus(req, res) {
    try {
        ...
        const pedido = await atualizarStatusPedido(pedidoId, status, req.user.id);
        return res.status(200).json({
