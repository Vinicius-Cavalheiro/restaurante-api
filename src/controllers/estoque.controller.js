import { buscarEstoque, listarEstoquePorUnidade, entradaEstoque, saidaEstoque, listarMovimentacoes, } from "../services/estoque.service.js";
...
export async function getEstoqueUnidade(req, res) {
    try {
        const unidadeId = Number(req.params.unidadeId);
...
export async function entrada(req, res) {
    try {
        ...
        if (!Number.isInteger(Number(unidadeId)) ||
            Number(unidadeId) <= 0 ||
        ...
        const estoque = await entradaEstoque(Number(unidadeId), Number(produtoId), Number(quantidade), req.user.id);
        return res.status(200).json({
...
export async function saida(req, res) {
    try {
        ...
        const estoque = await saidaEstoque(Number(unidadeId), Number(produtoId), Number(quantidade), req.user.id);
        return res.status(200).json({
...
export async function getMovimentacoes(req, res) {
    try {
        ...
        const movimentacoes = unidadeId !== undefined
